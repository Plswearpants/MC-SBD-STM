...
%% Load pre-generated synthetic datasets (repo-relative artifact root)
% Prefer artifacts/synthetic/datasets; fall back to examples/results for legacy.
artifact_candidates = { ...
    fullfile(repo_root, 'artifacts', 'synthetic', 'datasets'), ...
...
if isempty(resolved)
    % Keep relative fallback for users who cd into examples/
    syn_loc_try = fullfile('results', 'synthetic_datasets', syn_loc);
    if exist(syn_loc_try, 'file')
        resolved = syn_loc_try;
    else
        error('Dataset %s not found under artifacts/synthetic/datasets or examples/results/synthetic_datasets.', syn_loc);
    end
end
syn_loc = resolved;
load(syn_loc);
