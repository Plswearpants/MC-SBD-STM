...
ldos_candidates = { ...
    fullfile(repo_root, 'examples', 'example_data', 'LDoS_multi_1_defects_20260325_105344.mat'), ...
    fullfile(repo_root, 'examples', 'example_data', 'LDoS_sim.mat'), ...
    fullfile('example_data', 'LDoS_multi_1_defects_20260325_105344.mat') ...
    };
ldos_path = '';
for i = 1:numel(ldos_candidates)
    if exist(ldos_candidates{i}, 'file')
        ldos_path = ldos_candidates{i};
        break;
    end
end
if isempty(ldos_path)
    error('LDoS simulation mat not found under examples/example_data.');
end
load(ldos_path);
...
save_dir = fullfile(repo_root, 'artifacts', 'synthetic', 'datasets');
