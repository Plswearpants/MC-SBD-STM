...
%% Measure per-kernel per-energy SNR from measurement Y
% This script asks for n square windows on Y and computes SNR for each
% (window, energy-slice) pair using:
%   SNR = p2v / sigma_noise
% where p2v is estimated from center-cut peak prominence in each window.

if ~exist('estimateMeasurementSNRFromWindows', 'file')
    script_dir = fileparts(mfilename('fullpath'));
    addpath(fullfile(script_dir, '..', 'Dong_func'));
end
...
n = input('Enter number of kernel windows n: ');
while ~isscalar(n) || n < 1 || round(n) ~= n
    n = input('Please enter a positive integer for n: ');
end
n = double(n);
...
assignin('base', 'snr_roi_positions', roi_positions);
assignin('base', 'snr_noise_roi_position', noise_roi_position);
fprintf(['Saved to workspace variables: snr_matrix, snr_list, snr_details, snr_roi_positions, ' ...
    'snr_noise_roi_position\n']);

%% Visualization: SNR vs energy (one figure per kernel)
energy_axis = 1:e;
plot_snr_vs_energy(energy_axis, snr_matrix);

%% Visualization: sigma_noise vs energy
figure('Name', 'Sigma Noise vs Energy', 'Position', [140, 140, 900, 420]);
plot(energy_axis, snr_details.noise_std_per_slice, '-o', ...
    'LineWidth', 1.3, 'MarkerSize', 5, 'Color', [0.15 0.55 0.85], ...
    'MarkerFaceColor', [0.15 0.55 0.85]);
xlabel('Energy Slice');
ylabel('\sigma_{noise}');
title('\sigma_{noise} from noise ROI vs Energy');
grid on;

%% Optional: evaluate algorithm output kernels with same sigma_noise
compare_output = input('Estimate SNR for algorithm output kernels using this sigma_noise? (y/n, default n): ', 's');
output_kernels = squeeze(Aout(:,:,:,1:2));
if strcmpi(strtrim(compare_output), 'y')
    normalization_mode = 'rms'; % size-invariant normalization for apples-to-apples comparison

    drawn_kernel_stacks = extract_drawn_kernel_stacks(Y, roi_positions);
    [drawn_kernel_snr_norm_matrix, drawn_kernel_snr_norm_details] = estimateKernelSNRWithFixedNoise( ...
        drawn_kernel_stacks, snr_details.noise_std_per_slice, normalization_mode);

    [output_kernel_snr_matrix, output_kernel_snr_details] = estimateKernelSNRWithFixedNoise( ...
        output_kernels, snr_details.noise_std_per_slice, normalization_mode);

    assignin('base', 'output_kernel_snr_matrix', output_kernel_snr_matrix);
    assignin('base', 'output_kernel_snr_details', output_kernel_snr_details);
    assignin('base', 'output_kernel_snr_list', reshape(output_kernel_snr_matrix, 1, []));
    assignin('base', 'drawn_kernel_snr_norm_matrix', drawn_kernel_snr_norm_matrix);
    assignin('base', 'drawn_kernel_snr_norm_details', drawn_kernel_snr_norm_details);
    assignin('base', 'drawn_kernels_normalized', drawn_kernel_snr_norm_details.normalized_kernels);
    assignin('base', 'output_kernels_normalized', output_kernel_snr_details.normalized_kernels);
    fprintf(['Saved to workspace variables: output_kernel_snr_matrix, ' ...
        'output_kernel_snr_details, output_kernel_snr_list, ' ...
        'drawn_kernel_snr_norm_matrix, drawn_kernel_snr_norm_details, ' ...
        'drawn_kernels_normalized, output_kernels_normalized\n']);

    disp('output_kernel_snr_matrix (rows=output kernels, cols=energy slices):');
    disp(output_kernel_snr_matrix);

    disp('drawn_kernel_snr_norm_matrix (rows=drawn kernels, cols=energy slices):');
    disp(drawn_kernel_snr_norm_matrix);

    % 0) P2V-only enhancement (noise-ROI independent metric)
    % Use same normalization mode on both sides for apples-to-apples comparison.
    [~, drawn_p2v_details] = estimateKernelSNRWithFixedNoise(drawn_kernel_stacks, ones(1, e), normalization_mode);
    [~, output_p2v_details] = estimateKernelSNRWithFixedNoise(output_kernels, ones(1, e), normalization_mode);
    drawn_p2v_matrix = drawn_p2v_details.p2v_matrix;
    output_p2v_matrix = output_p2v_details.p2v_matrix;
    p2v_enhancement_factor = safe_ratio(output_p2v_matrix, drawn_p2v_matrix);

    assignin('base', 'drawn_p2v_matrix', drawn_p2v_matrix);
    assignin('base', 'output_p2v_matrix', output_p2v_matrix);
    assignin('base', 'p2v_enhancement_factor', p2v_enhancement_factor);
    fprintf(['Saved to workspace variables: drawn_p2v_matrix, output_p2v_matrix, ' ...
        'p2v_enhancement_factor\n']);
    disp('p2v_enhancement_factor = output_p2v_matrix ./ drawn_p2v_matrix');
    disp(p2v_enhancement_factor);
    plot_snr_factor_vs_energy(energy_axis, p2v_enhancement_factor, ...
        'P2V Enhancement Factor (noise-independent): output/drawn');

    % SNR factor (post-normalization comparison): output / drawn
    snr_factor_postnorm = safe_ratio(output_kernel_snr_matrix, drawn_kernel_snr_norm_matrix);
    assignin('base', 'snr_factor_postnorm', snr_factor_postnorm);
    disp('snr_factor_postnorm = output_kernel_snr_matrix ./ drawn_kernel_snr_norm_matrix');
    disp(snr_factor_postnorm);

    % 1) Post-normalization SNR (still compared to the same raw-data sigma_noise)
    plot_output_vs_drawn_snr(energy_axis, drawn_kernel_snr_norm_matrix, output_kernel_snr_matrix, ...
        'Post-normalization SNR (raw \sigma_{noise})');
    plot_snr_factor_vs_energy(energy_axis, snr_factor_postnorm, ...
        'SNR Factor (Post-normalization): output/drawn');

    % 2) Reference-aligned SNR: apply same scaling to drawn/output so
    %    drawn normalized SNR matches exact unnormalized drawn SNR baseline.
    [drawn_kernel_snr_scaled_matrix, output_kernel_snr_scaled_matrix, snr_scale_factors] = ...
        scale_snr_to_drawn_reference(snr_matrix, drawn_kernel_snr_norm_matrix, output_kernel_snr_matrix);

    assignin('base', 'drawn_kernel_snr_scaled_matrix', drawn_kernel_snr_scaled_matrix);
    assignin('base', 'output_kernel_snr_scaled_matrix', output_kernel_snr_scaled_matrix);
    assignin('base', 'snr_scale_factors', snr_scale_factors);
    snr_factor_reference = safe_ratio(output_kernel_snr_scaled_matrix, snr_matrix);
    assignin('base', 'snr_factor_reference', snr_factor_reference);
    fprintf(['Saved to workspace variables: drawn_kernel_snr_scaled_matrix, ' ...
        'output_kernel_snr_scaled_matrix, snr_scale_factors, snr_factor_postnorm, snr_factor_reference\n']);

    disp('snr_scale_factors (per kernel):');
    disp(snr_scale_factors);
    disp('snr_factor_reference = output_kernel_snr_scaled_matrix ./ snr_matrix');
    disp(snr_factor_reference);

    plot_output_vs_drawn_snr(energy_axis, drawn_kernel_snr_scaled_matrix, output_kernel_snr_scaled_matrix, ...
        'Reference-aligned SNR (drawn matched to unnormalized baseline)');
    plot_snr_factor_vs_energy(energy_axis, snr_factor_reference, ...
        'SNR Factor (Reference-aligned): output/drawn_exact');
end
...
function roi_positions = select_square_windows(slice_img, n)
    ...
    for k = 1:n
        ...
        center_handle = plot(nan, nan, 'r+', 'MarkerSize', 9, 'LineWidth', 1.4);
        update_center_indicator(h, center_handle);
        addlistener(h, 'MovingROI', @(src, evt) update_center_indicator(src, center_handle)); %#ok<NASGU>
        addlistener(h, 'ROIMoved', @(src, evt) update_center_indicator(src, center_handle)); %#ok<NASGU>
        wait(h);
        ...
        cy = pos(2) + (pos(4) - 1)/2;
        plot(cx, cy, 'y+', 'MarkerSize', 9, 'LineWidth', 1.4);
        text(pos(1), max(1, pos(2)-6), sprintf('#%d', k), ...
            'Color', 'y', 'FontWeight', 'bold');
        if isgraphics(center_handle)
...
function update_center_indicator(rect_roi, center_handle)
    if ~isvalid(rect_roi) || ~isgraphics(center_handle)
        return;
    end
    pos = rect_roi.Position;
    cx = pos(1) + pos(3)/2;
    cy = pos(2) + pos(4)/2;
    center_handle.XData = cx;
    center_handle.YData = cy;
end
...
function plot_snr_vs_energy(energy_axis, snr_matrix)
    num_kernels = size(snr_matrix, 1);
    for k = 1:num_kernels
        figure('Name', sprintf('Kernel %d: SNR vs Energy', k), ...
            'Position', [140 + 20*k, 120 + 20*k, 920, 520]);
        scatter(energy_axis, snr_matrix(k,:), 38, ...
            'Marker', 'o', ...
            'MarkerFaceColor', [0.1 0.45 0.9], ...
            'MarkerEdgeColor', [0.1 0.45 0.9], ...
            'DisplayName', sprintf('Kernel %d SNR', k));
        ylabel('SNR');
        ylim('auto');
        grid on;

        xlabel('Energy Slice');
        title(sprintf('Kernel %d: SNR vs Energy', k));
        legend('Location', 'best');
    end
end

function plot_output_vs_drawn_snr(energy_axis, drawn_snr_matrix, output_snr_matrix, title_prefix)
    if nargin < 4 || isempty(title_prefix)
        title_prefix = 'SNR Comparison';
    end
    ...
    for k = 1:num_fig
        figure('Name', sprintf('SNR Comparison Kernel %d', k), ...
            'Position', [180 + 15*k, 150 + 15*k, 920, 520]);
        hold on;

        if k <= num_drawn
            scatter(energy_axis, drawn_snr_matrix(k,:), 42, ...
                'Marker', 'o', ...
                'MarkerFaceColor', [0.12 0.48 0.86], ...
                'MarkerEdgeColor', [0.12 0.48 0.86], ...
                'DisplayName', 'Drawn kernel SNR');
        end

        if k <= num_output
            scatter(energy_axis, output_snr_matrix(k,:), 46, ...
                'Marker', 's', ...
                'MarkerFaceColor', [0.87 0.34 0.16], ...
                'MarkerEdgeColor', [0.87 0.34 0.16], ...
                'DisplayName', 'Output kernel SNR');
        end
...
function [drawn_scaled, output_scaled, scale_factors] = scale_snr_to_drawn_reference(drawn_raw, drawn_norm, output_norm)
    ...
    for k = 1:num_kernels
        ...
        if k <= size(drawn_raw,1)
            ...
            valid = isfinite(drawn_raw_k) & isfinite(drawn_norm_k) & (abs(drawn_norm_k) > eps);
            if any(valid)
                % Least-squares scalar so alpha*drawn_norm ~= drawn_raw.
                alpha = sum(drawn_raw_k(valid) .* drawn_norm_k(valid)) / sum(drawn_norm_k(valid).^2);
...
function plot_snr_factor_vs_energy(energy_axis, snr_factor_matrix, fig_title_prefix)
    num_kernels = size(snr_factor_matrix, 1);
    for k = 1:num_kernels
        figure('Name', sprintf('%s - Kernel %d', fig_title_prefix, k), ...
            'Position', [210 + 15*k, 180 + 15*k, 920, 460]);
        scatter(energy_axis, snr_factor_matrix(k,:), 40, ...
            'Marker', 'd', ...
            'MarkerFaceColor', [0.3 0.6 0.25], ...
            'MarkerEdgeColor', [0.2 0.45 0.2]);
        hold on;
        yline(1, '--', 'LineWidth', 1.1, 'Color', [0.4 0.4 0.4], 'DisplayName', 'Parity (1x)');
        hold off;
        grid on;
        xlabel('Energy Slice');
        ylabel('SNR Factor (output/drawn)');
        title(sprintf('%s - Kernel %d', fig_title_prefix, k));
    end
end
...
function drawn_kernel_stacks = extract_drawn_kernel_stacks(Y, roi_positions)
    ...
    for k = 1:num_kernels
        ...
        x = round(pos(1));
        y = round(pos(2));
        width = max(2, round(pos(3)));
        height = max(2, round(pos(4)));
...
function output_kernels = get_output_kernels_input()
    var_name = input(['Enter workspace variable name for output kernels ' ...
        '(empty to load from MAT file): '], 's');
    if ~isempty(strtrim(var_name))
        if evalin('base', sprintf('exist(''%s'',''var'')', var_name))
            output_kernels = evalin('base', var_name);
    ...
    [fname, fpath] = uigetfile('*.mat', 'Select MAT file containing output kernels');
    if isequal(fname, 0)
        error('No MAT file selected for output kernels.');
    end
    S = load(fullfile(fpath, fname));
    vars = fieldnames(S);
    if isempty(vars)
        error('Selected MAT file contains no variables.');
    end
    fprintf('Variables found in MAT file:\n');
    disp(vars);
    pick_name = input('Enter variable name to use as output kernels: ', 's');
    if ~isfield(S, pick_name)
        error('Variable "%s" not found in selected MAT file.', pick_name);
    end
    output_kernels = S.(pick_name);
