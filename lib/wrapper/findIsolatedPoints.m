...
%   See also: alignActivationMaps, padKernels
    ...
    % For 'kernel_sizes_all', kernels vary in size per slice. Pack each kernel
    % into a [H W num_slices] block, center-padded per kernel to the max slice
    % size, so A0_used is a cell VECTOR (one entry per kernel). This matches the
    % padded initial guess (IP01A) and the all-slice solver's kernel_sizes
    % (max over slices), and satisfies computeQualityMetrics, which requires A0
    % to be a cell vector. Existing (noisy) data.A0 values are preserved;
    % zero-padding leaves the Frobenius norm unchanged, so kernels stay on the
    % oblique sphere.
    if strcmp(target_kernel_size_type, 'kernel_sizes_all')
        num_slices_A0 = size(data.A0, 1);
        A0_used = cell(params.num_kernels, 1);
        for k = 1:params.num_kernels
            Hs = zeros(num_slices_A0, 1);
            Ws = zeros(num_slices_A0, 1);
            for s = 1:num_slices_A0
                Hs(s) = size(data.A0{s,k}, 1);
                Ws(s) = size(data.A0{s,k}, 2);
            end
            H = max(Hs);
            W = max(Ws);
            A0_used{k} = zeros(H, W, num_slices_A0);
            for s = 1:num_slices_A0
                src = data.A0{s,k};
                pad_r = floor((H - size(src, 1)) / 2);
                pad_c = floor((W - size(src, 2)) / 2);
                A0_used{k}(pad_r+(1:size(src,1)), pad_c+(1:size(src,2)), s) = src;
            end
        end
    else
