...
%           without prompting (both defaults must then be set).
    ...
    % Normalize the volume once, then extract the already-normalized reference slice.
    [Y, ~, ~, noise_roi] = normalizeBackgroundToZeroMean3D(Y, 'dynamic', params.refSlice.ref_slice);
    Y = proj2oblique(Y);
