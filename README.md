...
| Decompose a real volume (`Y` → `A_out, X_out`)| [`run entrance/scripts/real/real_block.m`](run%20entrance/scripts/real/real_block.m) | Paper-style figures: [`paper/figures/Main Figures/plotting_paper.m`](paper/figures/Main%20Figures/plotting_paper.m) | same project folder as preprocess, if you point `params.project.existing_path` at it |
| Generate one synthetic dataset and run MCSBD | [`run entrance/scripts/synthetic/synthetic_data.m`](run%20entrance/scripts/synthetic/synthetic_data.m) | Follow the script’s GD / DS / DA / VR cells | `run entrance/projects/synthetic_<timestamp>/` |
...
Each script includes more detailed instructions. 
