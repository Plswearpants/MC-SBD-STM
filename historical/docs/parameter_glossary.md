...
Applies to [`run entrance/scripts/synthetic/synthetic_data.m`](../run%20entrance/scripts/synthetic/synthetic_data.m) and its wrappers.
Living layout: [`REPO_STRUCTURE.md`](REPO_STRUCTURE.md).
...
| **LDoS_path** | string | - | - | *optional* | Path to LDoS simulation `.mat`. Empty resolves `store/synthetic/ldos/` (then `examples/`). |
...
| **boundary_padded** | logical [1×K] | - | True when IS01A had to keep a near-edge peak (IP01A zero-pads that kernel crop) |
