## MC-SBD-STM script standardization

Living repo map (streams, DAG, migration log): [`REPO_STRUCTURE.md`](REPO_STRUCTURE.md), [`REPO_STRUCTURE_DAG.md`](REPO_STRUCTURE_DAG.md), [`../historical/history/repo_reorg.md`](../historical/history/repo_reorg.md).

This file defines a **minimal, high-reliability** standard for scripts in `scripts/` (`real_preprocess.m`, `real_block.m`, `synthetic_data.m`, phase-space runners). The goal is to make scripts **repeatable**, **auditable**, and easy to refactor without changing scientific intent.
...
- Script names:
  - Official trunks: `real_preprocess.m`, `real_block.m`, `synthetic_data.m`, plus the phase-space trio under `run entrance/scripts/phase_space/`.
  - Keep long, exploratory notebooks out of those trunks (move into `examples/` or `historical/`).
- Block IDs:
  - Follow the `synthetic_data.m` convention: `ABXXZ` (Category/Subcategory/Number/Variant).
...
### Settled layout (do not reopen here)

- Real and synthetic session trees live under `projects/` (`real_<ts>/`, `synthetic_<ts>/`).
- Reusable payloads live under `store/`; the manuscript set is `paper/freeze/`.
- Checkpoint-tree UI (pick project → pick node) remains a design goal; trunks today use PRESETS + wrappers, not a full tree browser.
