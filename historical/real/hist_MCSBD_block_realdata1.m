...
[Y, ~, ~, noise_roi] = normalizeBackgroundToZeroMean3D(Y, 'dynamic', params.ref_slice);  % background ROI reused for eta
Y = proj2oblique(Y);

% Extract reference slice (already normalized with the volume) and initialize kernel
...
if ~exist('noise_roi', 'var') || isempty(noise_roi)
    if isfield(params, 'noise_roi') && ~isempty(params.noise_roi)
        noise_roi = params.noise_roi;
    else
        error('noise_roi is missing. Re-run the reference-slice normalize block first.');
    end
end
eta_data = estimate_noise3D(Y_ref, 'std', noise_roi);
params.noise_roi = noise_roi;
...
elseif isfield(params, 'noise_roi') && ~isempty(params.noise_roi)
    eta_data3d = estimate_noise3D(Y, 'std', params.noise_roi);
else
