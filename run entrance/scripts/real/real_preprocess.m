...
% PRESETS: User-configurable parameters
% -------------------------------------------------------------------------
cfg.load.data_file       = [];   % raw .3ds file name/path ([] = pick via UI, starts in store/real/raw)
cfg.load.smoothing_sigma = 3;    % smoothing sigma for the current data
cfg.io.sample            = 'ZrSiTe';  % seeds store/real/raw/<sample>/ and later promote folder
...
%        preprocessing_params, ...), the handoff file that real_block.m
%        picks up through its LR01B variable picker
%  If promote_to_store is true, a copy also lands in
%  store/real/processed/<sample>_<sample_date>/ (the reusable library index).
%
%  Dependencies: none
%
% -------------------------------------------------------------------------
% PRESETS: User-configurable parameters
% -------------------------------------------------------------------------
cfg.io.run_label            = 'ZrSiTe0826';   % stem for the handoff .mat
cfg.io.sample               = 'ZrSiTe';       % material prefix for store/real/processed/
cfg.io.sample_date          = '0826';         % MMDD; folder is <sample>_<sample_date>
params.preprocessing.save_checkpoint  = true; % full resumable checkpoint
params.preprocessing.save_handoff     = true; % plain-variable file for real_block
params.preprocessing.promote_to_store = true; % copy handoff into store/real/processed/<sample>_<date>/
...
if params.preprocessing.save_handoff
    ...
    promote = isfield(params.preprocessing, 'promote_to_store') && ...
        params.preprocessing.promote_to_store;
    sample = '';
    sample_date = '';
    ...
    if ~isempty(sample) && ~isempty(sample_date)
        store_folder = [sample '_' sample_date];
    else
        store_folder = sample_date;
    end
    if promote && ~isempty(store_folder) && exist('repo_root', 'var') && ~isempty(repo_root)
        store_dir = fullfile(repo_root, 'store', 'real', 'processed', store_folder);
        if ~exist(store_dir, 'dir')
            mkdir(store_dir);
        end
        [~, hf_name, hf_ext] = fileparts(handoff_file);
        store_file = fullfile(store_dir, [hf_name hf_ext]);
        copyfile(handoff_file, store_file);
        fprintf('Promoted handoff to %s\n', store_file);
        LOGcomment = sprintf("Promoted to store: %s", store_file);
        LOGcomment = logUsedBlocks(log.path, log.file, "  ^  ", LOGcomment, 0);
    end
end
...
fprintf('\nNext step: run run entrance/scripts/real/real_block.m and set\n');
