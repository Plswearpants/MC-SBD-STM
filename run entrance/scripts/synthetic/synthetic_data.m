...
%  PARAMETER GLOSSARY:
%  ===================
%  See historical/docs/parameter_glossary.md (internal; parked).
%
% =========================================================================
...
% -------------------------------------------------------------------------
params.synGen.SNR = 3;                        % Signal-to-noise ratio
...
params.synGen.LDoS_path = '';  % '' = store/synthetic/ldos (file picker if missing)
...
% Wrappers return hierarchical params; this block uses the flat view.
...
LOGcomment = sprintf("Window: %s, Interactive: %d, Matrix: %d", ...
    formatWindowType(window_type_proliferation), interactive_size_adjust, use_matrix_format);
...
% Slice-specific proliferation can yield different kernel sizes; pad to a
% common [H W] per kernel so the all-slice solver can stack them.
if use_matrix_format
    A1_all_matrix = cell(params.num_kernels, 1);
    for k = 1:params.num_kernels
        Hs = zeros(params.num_slices, 1);
        Ws = zeros(params.num_slices, 1);
        for s = 1:params.num_slices
            Hs(s) = size(A1_all{s,k}, 1);
            Ws(s) = size(A1_all{s,k}, 2);
        end
        H = max(Hs);
        W = max(Ws);
        A1_all_matrix{k} = zeros(H, W, params.num_slices);
        for s = 1:params.num_slices
            src = A1_all{s,k};
            pad_r = floor((H - size(src, 1)) / 2);
            pad_c = floor((W - size(src, 2)) / 2);
            A1_all_matrix{k}(pad_r+(1:size(src,1)), pad_c+(1:size(src,2)), s) = src;
            A1_all_matrix{k}(:,:,s) = proj2oblique(A1_all_matrix{k}(:,:,s));
        end
    end
    fprintf('Converted to matrix format (%d kernels, padded per kernel to max slice size):\n', ...
        params.num_kernels);
    for k = 1:params.num_kernels
        fprintf('  Kernel %d: [%dx%dx%d]\n', k, ...
            size(A1_all_matrix{k},1), size(A1_all_matrix{k},2), params.num_slices);
    end
    fprintf('\n');
end
...
initial_iteration = 2;             % Inner Manopt/FISTA iterations at start
maxIT_allslice = 10;               % Max outer iterations for all-slice decomposition
...
num_kernels_allslice = size(kernel_sizes_used, 1);
...
% Reference-slice results now live in data.mcsbd_slice (decomposeReferenceSlice
% wrapper), so rebuild the solver params from the flat params here. kplus is
% sized for the all-slice (padded) kernel sizes, not the reference slice.
params_allslice = struct();
if isscalar(params.lambda1)
    params_allslice.lambda1 = repmat(params.lambda1, 1, num_kernels_allslice);
elseif numel(params.lambda1) == num_kernels_allslice
    params_allslice.lambda1 = reshape(params.lambda1, 1, []);
else
    error('DA01A: lambda1 must be scalar or length %d (got %d).', ...
        num_kernels_allslice, numel(params.lambda1));
end
params_allslice.phase2  = params.phase2_enable;
if isscalar(params.lambda2)
    params_allslice.lambda2 = repmat(params.lambda2, 1, num_kernels_allslice);
elseif numel(params.lambda2) == num_kernels_allslice
    params_allslice.lambda2 = reshape(params.lambda2, 1, []);
else
    error('DA01A: lambda2 must be scalar or length %d (got %d).', ...
        num_kernels_allslice, numel(params.lambda2));
end
params_allslice.nrefine = params.nrefine;
params_allslice.kplus   = ceil(params.kplus_factor * kernel_sizes_used);
params_allslice.xpos    = params.xpos;
params_allslice.getbias = params.getbias;
params_allslice.Xsolve  = params.Xsolve_method;
...
if use_reference_init
    params_allslice.xinit = cell(1, num_kernels_allslice);
    for k = 1:num_kernels_allslice
        params_allslice.xinit{k}.X = data.mcsbd_slice.X(:,:,k);
        b_temp = data.mcsbd_slice.extras.phase1.biter(k);
...
% Keep all-slice outputs in data for downstream wrappers (VR01A/saveRun).
data.mcsbd_block = allslice_results;

fprintf('Multi-slice decomposition complete.\n\n');

% LOG: All-slice results
LOGcomment = sprintf("Completed in %.2fs for %d slices", allslice_time, params.num_slices);
LOGcomment = logUsedBlocks(log.path, log.file, "  ^  ", LOGcomment, 0);

% Clear preset variables
clearvars use_reference_init show_allslice_progress maxIT_allslice initial_iteration
clearvars kernel_sizes_used Y_used A1_used dispfun_allslice
clearvars params_allslice b_temp allslice_time n k num_kernels_allslice
...
%
%  Dependencies: visualizeResultsSynthetic.m
...
use_visualization_ui = true;        % Interactive UI (slice/param/item selectors)
...
% LOG: Block start
LOGcomment = sprintf("Visualize all: %d, Save: %d, UI: %d", ...
    visualize_all_slices, save_figures, use_visualization_ui);
LOGcomment = logUsedBlocks(log.path, log.file, "VR01A", LOGcomment, 0);

fprintf('Generating result visualizations...\n');

% Backward-compatibility: old workspaces may not have data.mcsbd_block yet.
if exist('allslice_results', 'var') && isstruct(allslice_results)
    run_data_for_viz = allslice_results;
else
    run_data_for_viz = [];
end

% Run synthetic-specific visualization using data + params only.
[data, viz_info] = visualizeResultsSynthetic(data, params, ...
    'run_data', run_data_for_viz, ...
    'use_ui', use_visualization_ui, ...
    'visualize_all_slices', visualize_all_slices, ...
    'save_figures', save_figures, ...
    'figure_dir', log.path, ...
    'figure_prefix', log.file);

% Log saved figures if requested
if save_figures && ~isempty(viz_info.saved_figures)
    for i = 1:numel(viz_info.saved_figures)
        [~, saved_name, saved_ext] = fileparts(viz_info.saved_figures{i});
        LOGcomment = sprintf("Saved figure: %s%s", saved_name, saved_ext);
        LOGcomment = logUsedBlocks(log.path, log.file, "  ^  ", LOGcomment, 0);
    end
end
...
LOGcomment = sprintf("Visualized %d slices", numel(viz_info.slices_visualized));
LOGcomment = logUsedBlocks(log.path, log.file, "  ^  ", LOGcomment, 0);

% Clear preset variables
clearvars visualize_all_slices save_figures use_visualization_ui viz_info
clearvars s k i saved_name saved_ext run_data_for_viz
...
if save_workspace || save_results_only
    ...
    if save_results_only
        % Save only essential results. Reference-slice results live in
        % data.mcsbd_slice; all-slice results in allslice_results.
        save(results_fullpath, 'data', 'params', ...
            'allslice_results', 'isolation_results', 'log', '-v7.3');
