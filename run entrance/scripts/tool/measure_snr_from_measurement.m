...
function Y = load_measurement_Y_if_needed()
    ...
    paths = repo_payload_paths();
    start_dir = first_existing_dir({paths.real_processed, paths.real_runs}, pwd);
    [fname, fpath] = uigetfile('*.mat', 'Select MAT file containing measurement Y', start_dir);
    if isequal(fname, 0)
...
function output_kernels = get_output_kernels_input()
    ...
    paths = repo_payload_paths();
    start_dir = first_existing_dir({paths.real_runs, paths.real_processed}, pwd);
    [fname, fpath] = uigetfile('*.mat', 'Select MAT file containing recovered kernels', start_dir);
