...
%% Load pre-generated synthetic datasets (repo-relative store)
% Prefer store/phase_space/datasets; fall back to legacy locations.
paths = repo_payload_paths(repo_root);
artifact_candidates = { ...
    paths.phase_datasets, ...
    fullfile(repo_root, 'synthetic results'), ...
    fullfile(repo_root, 'examples', 'results', 'synthetic_datasets') ...
    };
syn_loc = 'synthetic_datasets_20260403_233155.mat';
resolved = '';
for i = 1:numel(artifact_candidates)
    cand = fullfile(artifact_candidates{i}, syn_loc);
    if exist(cand, 'file')
        resolved = cand;
        break;
    end
...
if isempty(resolved)
    syn_loc_try = fullfile('results', 'synthetic_datasets', syn_loc);
    if exist(syn_loc_try, 'file')
        resolved = syn_loc_try;
    else
        error('Dataset %s not found under store/phase_space/datasets.', syn_loc);
    end
end
syn_loc = resolved;
load(syn_loc);

[~, dataset_stem] = fileparts(syn_loc);
dataset_stem = regexprep(dataset_stem, '_init_(activation|random|isolated)$', '');
run_out_dir = fullfile(paths.phase_parallel, dataset_stem);
if ~exist(run_out_dir, 'dir')
    mkdir(run_out_dir);
end
...
% Create config directory (repo-rooted, independent of cwd)
config_dir = fullfile(repo_root, 'config', 'worker_configs');
...
copyfile(fullfile(repo_root, 'config', 'Xsolve_config.mat'), ...
    fullfile(param_dir, 'Xsolve_config_tunable.mat'));
copyfile(fullfile(repo_root, 'config', 'Asolve_config.mat'), ...
...
%parfor n = 1:num_datasets
    try
        ...
        % Save results
        filename = fullfile(run_out_dir, sprintf('SBD_parallel_dataset%d_optimal.mat', n));
