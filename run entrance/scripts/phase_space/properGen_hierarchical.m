...
loaded_ldos = load(ldos_path);
if isfield(loaded_ldos, 'LDoS_result')
    LDoS_sim = loaded_ldos.LDoS_result;
elseif isfield(loaded_ldos, 'LDoS_sim')
    LDoS_sim = loaded_ldos.LDoS_sim;
else
    error('No LDoS_result or LDoS_sim field in %s', ldos_path);
end
if ~isfield(loaded_ldos, 'N')
    error('N not found in %s. This parameter is required.', ldos_path);
end
N = loaded_ldos.N;
...
save_dir = fullfile(repo_root, 'store', 'phase_space', 'datasets');
